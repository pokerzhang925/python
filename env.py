...
def create_env(opt):
    ...
    if opt.envs == 'pong':
        if opt.ncolumns == 1:
            envs = [
                NormalizedEnv(AtariRescale(PongHFlip(gym.make('PongDeterministic-v4'))))
                # NormalizedEnv(AtariRescale(PongZoom(gym.make('PongDeterministic-v4'))))
                # NormalizedEnv(AtariRescale(gym.make('PongDeterministic-v4')))
            ]
        elif opt.ncolumns == 2:
            envs = [
                #NormalizedEnv(PongNoisy(AtariRescale(gym.make('PongDeterministic-v4')))),
                #NormalizedEnv(AtariRescale(gym.make('PongDeterministic-v4')))
                NormalizedEnv(AtariRescale(gym.make('PongDeterministic-v4'))),
                NormalizedEnv(AtariRescale(PongZoom(gym.make('PongDeterministic-v4'))))
            ]
        ...
        if opt.ncolumns == 1:
            ...
            envs = [
                NormalizedEnv(AtariRescale(gym.make('SeaquestDeterministic-v4'))),
                NormalizedEnv(AtariRescale(gym.make('RiverraidDeterministic-v4'))),
                NormalizedEnv(AtariRescale(gym.make('BoxingDeterministic-v4')))
            ]
        elif opt.ncolumns == 4:
            envs = [
...
class AtariRescale(gym.ObservationWrapper):
    def __init__(self, env=None):
        ...
        # Box(low, high, shape)
        self.observation_space = Box(0.0,1.0,[84,84])#Box(0.0, 1.0, [1, 84, 84])

    def observation(self, frame):
        # rescale to 160 x 160
        frame = frame[34:34 + 160, :160] #34到194
        # resize to 84 x 84
...
class PongHFlip(gym.Wrapper):
    ...
    def reset(self):
        ...
        observation = cv2.flip(observation, 1)# >0 水平翻转，0 垂直翻转， <0 水平垂直方向均翻转
